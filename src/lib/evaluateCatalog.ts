...
// ── Azure: `name` é o GUID OU o nome de exibição, conforme a origem ────────
//
// Os dois formatos oficiais discordam sobre o significado da mesma chave:
//
//   ARM / az CLI            "name": "<guid>",  "roleName": "Reader"
//   Get-AzRoleDefinition    "Name": "Reader",  "Id": "<guid>"
//
// Um alias de caixa não resolve — `Name` e `name` são a MESMA chave em
// minúsculas e querem dizer coisas diferentes. O que desempata é a forma do
// valor: GUID é identificador, o resto é nome. Sem isto, o print de 21/08
// mostrava "API Management Workspace Reader" impresso como se fosse o
// identificador da role, e o matching procurava esse texto entre os GUIDs.

const GUID_RE = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i

/** O GUID da roleDefinition, venha ele solto ou no fim de um caminho ARM. */
function azureRoleId(j: Record<string, any>): string | null {
  const candidatos = [
    j.properties?.roleDefinitionId,
    typeof j.name === 'string' && GUID_RE.test(j.name) ? j.name : null,
    typeof j.id === 'string' ? j.id.split('/').pop() : null,
  ]
  for (const c of candidatos) if (typeof c === 'string' && GUID_RE.test(c)) return c
  // Nenhum GUID? Devolve o `id` cru — é melhor mostrar o caminho ARM inteiro
  // do que não mostrar identificador nenhum.
  return typeof j.id === 'string' ? j.id : null
}

/** O nome de exibição, nunca o GUID. */
function azureRoleName(j: Record<string, any>): string | null {
  const candidatos = [
    j.properties?.roleName, j.roleName, j.displayName,
    typeof j.name === 'string' && !GUID_RE.test(j.name) ? j.name : null,
  ]
...
function extractRawPermissions(cloud: EvaluateCloud, j: Record<string, any>): EvaluatedPermission[] {
  try {
    switch (cloud) {
      ...
      case 'azureRbac': {
        // Duas formas: a aninhada (`permissions[0].actions`, do ARM e do az
        // CLI) e a ACHATADA do Get-AzRoleDefinition, em que `Actions` e
        // `DataActions` saem na raiz. Ler só a primeira fazia a role do print
        // de 21/08 chegar ao avaliador com zero permissões — e, sem permissão,
        // a avaliação por permissões também não tinha o que dizer.
        const perm = j.properties?.permissions?.[0] ?? j.permissions?.[0]
        const actions: string[] = [
          ...(perm?.actions ?? j.actions ?? []),
          ...(perm?.dataActions ?? j.dataActions ?? []),
        ]
        // `notActions` fica de fora: é exclusão, não concessão.
        return actions.map((a) => ({ name: a }))
      }
      case 'aws': {
...
function matchAzure(j: Record<string, any>): { record: AzureRbacRole; matchedBy: MatchedBy } | null {
  const idCandidate = azureRoleId(j)
  if (idCandidate) {
    const byId = AZURE_ROLES.find((r) => r.id.toLowerCase() === idCandidate.toLowerCase())
    if (byId) return { record: byId, matchedBy: 'id' }
  }
  const nameCandidate = azureRoleName(j)
  if (nameCandidate) {
    const byName = AZURE_ROLES.find((r) => r.name.toLowerCase() === nameCandidate.toLowerCase())
    if (byName) return { record: byName, matchedBy: 'name' }
  }
  return null
}
    ...
    return {
      ...
      result: {
        ...
        tier: {
          ...
          color: '#6b7280',
          bg: '#6b728018',
          // Não dizer "não é possível determinar o tier": logo abaixo desta
          // frase o site mostra um tier estimado a partir das permissões, e as
          // duas coisas juntas se contradiziam na mesma tela.
          justification: 'Esta role não está no catálogo do site, então a classificação catalogada não se aplica a ela. A estimativa a partir das permissões que você colou está logo abaixo.',
        },

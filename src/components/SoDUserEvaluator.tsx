    ...
    <div className="p-6 max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-5 gap-6 overflow-y-auto flex-1">
      ...
      <div className="lg:col-span-2 flex flex-col gap-3">
        ...
        <textarea
          ...
          spellCheck={false}
          className="w-full font-mono text-[12px] leading-relaxed p-3 rounded-lg border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900 text-gray-800 dark:text-gray-100 resize-y focus:outline-none focus:ring-1 focus:ring-[#0078d4]"
        />

        <div>
          <label className="text-[11px] font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-1.5 block">Cloud</label>
      ...
      <div className="lg:col-span-3">
        {!result && (
          <div className="flex flex-col items-center justify-center h-full min-h-[400px] text-center px-6 py-16 border border-dashed border-gray-200 dark:border-gray-800 rounded-xl">
            <Sparkles size={28} className="text-gray-300 dark:text-gray-700 mb-3" />
            <p className="text-[14px] font-medium text-gray-600 dark:text-gray-300 mb-1">Cole as roles atribuídas para começar</p>
          ...
          <div className="space-y-5">
            ...
            <div className="rounded-xl p-5 border" style={{
              ...
              <div className="flex items-center justify-between flex-wrap gap-3">
                <span
                  className={`text-[16px] font-bold px-4 py-2 rounded-lg ${SOD_RISK_META[result.riskLevel].pulse ? 'animate-pulse' : ''}`}
                  style={{ color: SOD_RISK_META[result.riskLevel].color, background: 'white' }}
                >
                  {SOD_RISK_META[result.riskLevel].label}
                </span>
                <button onClick={exportReport}
                  className="flex items-center gap-1.5 text-[12px] px-3 py-1.5 rounded-md border border-gray-300 dark:border-gray-600 bg-white/60 dark:bg-gray-900/60 hover:bg-white dark:hover:bg-gray-900 transition-colors">
            ...
            {result.conflicts.length > 0 && (
              <div>
                <p className="text-[11px] font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-2">Conflitos encontrados</p>
                    ...
                    return (
                      <div key={i} className="border border-gray-200 dark:border-gray-700 rounded-lg overflow-hidden">
                        <button onClick={() => toggleConflict(i)}
                          ...
                          <span className="flex-1 text-[12px] text-gray-700 dark:text-gray-300 flex items-center gap-1.5 flex-wrap">
                        ...
                        {isOpen && (
                          <div className="px-4 pb-4 pt-1 bg-gray-50/60 dark:bg-gray-900/40">
                            ...
                            <Link href={`/sod/rules/${c.rule.id}`} className="inline-block mt-3 text-[12px] text-[#0078d4] dark:text-[#85b7eb] hover:underline">
            ...
            {result.rolesNotFound.length > 0 && (
              <div className="rounded-lg p-3 bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-900/50">
                <div className="flex items-center gap-1.5 mb-1.5">
                  ...
                  <p className="text-[11px] font-semibold text-amber-700 dark:text-amber-400 uppercase tracking-wider">Roles não reconhecidas</p>
                </div>
                <p className="text-[11px] text-amber-700 dark:text-amber-400 mb-2">Estas roles não foram encontradas na base — validar manualmente.</p>
                <ul className="text-[12px] text-gray-600 dark:text-gray-400 space-y-1.5">
                            ...
                            Você quis dizer: {suggestions.map((s, si) => (
                              <span key={s.slug}>
                                ...
                                <button
                                  ...
                                  className="text-[#0078d4] dark:text-[#85b7eb] hover:underline"
                                >
                                  {s.name}
                                </button>
                              </span>
...
function Metric({ label, value }: { label: string; value: string | number }) {
  return (
    <div>
      <p className="text-[18px] font-bold text-gray-800 dark:text-gray-100">{value}</p>

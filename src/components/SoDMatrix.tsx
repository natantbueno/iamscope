  ...
  return (
    <div className="p-6 space-y-8 overflow-y-auto flex-1">
      ...
      <div className="flex items-center gap-2">
        <span className="text-[11px] text-gray-400 dark:text-gray-500 w-20 shrink-0">Cloud</span>
        {(['both', 'entra-id', 'azure-rbac'] as SoDCloudScope[]).map((c) => (
          <button key={c} onClick={() => { setCloudScope(c); setRoleA(null); setRoleB(null); setChecked(false) }}
            className={`text-[11px] px-2.5 py-1 rounded-full border font-medium transition-colors ${
              cloudScope === c ? 'bg-[#e8f1fb] dark:bg-[#0c2a47] text-[#0078d4] dark:text-[#85b7eb] border-[#9dc3e8] dark:border-[#185fa5]'
      ...
      <button onClick={verify} disabled={!roleA || !roleB}
        className="px-4 py-2 rounded-lg bg-[#0078d4] hover:bg-[#006cbe] disabled:opacity-40 disabled:cursor-not-allowed text-white text-[13px] font-medium transition-colors">
        Verificar Conflito
      </button>
      ...
      {checked && roleA && roleB && sameRole && (
        <div className="border border-amber-200 dark:border-amber-900/50 rounded-xl p-4 bg-amber-50/40 dark:bg-amber-950/20 flex items-center gap-2">
          ...
          <span className="text-[12px] text-amber-700 dark:text-amber-400">Selecione roles diferentes para verificar conflito.</span>
        </div>
      )}
      {checked && roleA && roleB && !sameRole && (
        result ? (
          <div className="border border-red-200 dark:border-red-900/50 rounded-xl p-5 bg-red-50/40 dark:bg-red-950/20">
            <div className="flex items-center gap-2 mb-4">
              ...
              <span className="text-[13px] font-bold uppercase tracking-wider text-red-600 dark:text-red-400">Conflito Detectado</span>
              <SoDSeverityBadge severity={result.severity} />
            </div>
            <SoDRuleDetailCard rule={result} compact />
            <Link href={`/sod/rules/${result.id}`} className="inline-block mt-4 text-[12px] text-[#0078d4] dark:text-[#85b7eb] hover:underline">
          ...
          <div className="border border-emerald-200 dark:border-emerald-900/50 rounded-xl p-5 bg-emerald-50/40 dark:bg-emerald-950/20">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-[13px] font-bold uppercase tracking-wider text-emerald-600 dark:text-emerald-400">Sem Conflito Registrado</span>
            </div>
            ...
            {otherConflicts.map((r) => {
              ...
              return (
                <Link key={r.id} href={`/sod/rules/${r.id}`}
                  className="flex items-center gap-2 text-[12px] px-3 py-1.5 rounded-lg border border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">
      ...
      <div>
          ...
          <table className="border-collapse text-[11px]">
              ...
              <tr>
                ...
                {MATRIX_ROLES.map((r) => (
                  <th key={r.name} className="p-1 align-bottom">
                    <div className="text-gray-500 dark:text-gray-400 font-medium whitespace-nowrap [writing-mode:vertical-rl] rotate-180 h-24 flex items-end justify-center">
                      {r.name}
                    </div>
                  </th>
                ))}
            ...
            <tbody>
              {MATRIX_ROLES.map((rowRole) => (
                <tr key={rowRole.name}>
                  <td className="p-1 text-right text-gray-500 dark:text-gray-400 font-medium whitespace-nowrap pr-2">{rowRole.name}</td>
                    ...
                    return (
                      <td key={cellId} className="p-0">
                        <Link
                          ...
                          title={rule ? rule.name : 'Sem conflito registrado'}
        ...
        {hoverCell && <p className="text-[11px] text-gray-400 mt-2">{hoverCell}</p>}
      </div>
    </div>
  )
}
...
function RoleCombobox({ label, cloudScope, query, onQueryChange, selected, onSelect }: {
  ...
  return (
    <div className="relative">
      <label className="text-[11px] font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-1.5 block">{label}</label>
      <div className="relative">
        ...
        <input
          ...
          className="w-full text-[13px] pl-8 pr-3 py-2 rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 text-gray-800 dark:text-gray-100 focus:outline-none focus:ring-1 focus:ring-[#0078d4]"
        />
      </div>
        ...
        <div className="absolute z-20 mt-1 w-full max-h-60 overflow-y-auto rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 shadow-lg">
          {results.map((r) => (
            <button key={`${r.cloud}-${r.slug}`}
              ...
              className="w-full text-left px-3 py-1.5 text-[12px] text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-800 flex items-center justify-between gap-2">
              <span className="truncate">{r.name}</span>
              <span className="text-[9px] text-gray-400 shrink-0">{r.cloud === 'entra-id' ? 'Entra ID' : 'Azure RBAC'}</span>
            </button>
          ))}
        </div>
      )}

...
const ROUTES: Record<Platform, Partial<Record<View, string>>> = {
  home:            {},
  // Páginas globais não têm views de plataforma — a sidebar delas só lista as
...
export default function AppShell({
  ...
  // Página global (ferramenta, /info, busca) é multi-cloud: nenhum menu de
  // plataforma se aplica. Sem este teste elas caíam no fallback `entraId` e a
  // sidebar mostrava Built-in Roles, Role Actions, PIM e o Enterprise Access
  // Model do Entra ID ao lado de uma tela que não é do Entra ID.
  //
  // O teste vem ANTES dos de cloud de propósito, e é seguro: nenhuma rota de
  // cloud começa com um destes prefixos (`/aws/reference` não começa com
  // `/reference`).
  const isGlobal    = GLOBAL_HREFS.some((href) => pathname === href || pathname.startsWith(`${href}/`))

  const platform: Platform = isGlobal
    ? 'global'
    : isGws
    ? 'googleWorkspace'
    : isAzureRbac
        ...
        */}
        ...
        <header className="bg-surface border-b border-line px-4 sm:px-6 py-3 flex items-center gap-3 sm:gap-4 shrink-0">
          <button
            ...
            className="md:hidden shrink-0 -ml-1 p-2 rounded-md text-fg-muted hover:text-gray-700 dark:hover:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-brand"
          >
            <Menu size={18} />
          </button>
          {headerBack && (
          ...
          {/* Tema e idioma subiram para a linha do menu de clouds (CloudNav):
              sao preferencias da pessoa, valem para o site inteiro, e aqui
              disputavam largura com o titulo da pagina. O que sobra nesta
              linha e o que pertence A PAGINA — exportar, selo de escopo. */}
          {headerActions && (

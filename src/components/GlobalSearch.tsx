...
// - Digitando em outra página → router.push para a listagem com ?q=, depois de
//   250ms sem digitar, e o foco é restaurado após a remontagem (flag em escopo
//   de módulo, que sobrevive à navegação client-side).
...
function GlobalSearchInner({ basePath }: { basePath: string }) {
  ...
  // Timer da navegação adiada — ver handleChange.
  const timerNavegacao = useRef<ReturnType<typeof setTimeout> | null>(null)

  useEffect(() => () => {
    if (timerNavegacao.current) clearTimeout(timerNavegacao.current)
  }, [])
  ...
  const handleChange = (val: string) => {
    ...
    if (normalizePath(pathname) === normalizePath(basePath)) {
      ...
      /**
       * Em outra página: navega para a listagem — mas só depois de uma pausa
       * na digitação.
       *
       * Sem a pausa, a navegação saía na PRIMEIRA tecla. Quando o destino
       * mora em outro layout (as sete ferramentas e a home vão para /search),
       * a remontagem descarta o input e as teclas digitadas durante a
       * transição caem no vazio: digitar "admin" em /sod chegava em
       * /search?q=ain. Dentro da mesma cloud isso não aparecia, porque o
       * layout é o mesmo e o campo sobrevive.
       *
       * 250ms também elimina o efeito colateral de um `push` por tecla — o
       * botão "voltar" precisava de cinco cliques para sair de uma busca de
       * cinco letras.
       */
      if (timerNavegacao.current) clearTimeout(timerNavegacao.current)
      if (!val) return

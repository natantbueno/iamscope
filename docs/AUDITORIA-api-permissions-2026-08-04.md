...
**Data:** 2026-08-04
**Escopo:** medir a diferença entre os 854 registros de `src/data/apiPermissions.ts` e os ~2.681 que aparecem em listagens de terceiro, para decidir se é lacuna real ou critério de contagem diferente.

## Veredito

São as duas coisas, em proporções diferentes:

- **A comparação direta com os ~2.681 é inválida.** Essas listagens cobrem várias resource APIs (costumam ter filtro por `targetAppId`, ex.: `00000003-0000-0000-c000-000000000000` para o Microsoft Graph). Nosso dataset é 100% Microsoft Graph — `resource` é `'Microsoft Graph'` nos 854 registros. Comparar os dois totais é somar maçãs com laranjas.
...
1. Corrigir o texto do site — se algum lugar compara com os ~2.681 de listagem de terceiro, a comparação está errada por construção. O número honesto para "cobertura do Microsoft Graph" é 854/1.504.
